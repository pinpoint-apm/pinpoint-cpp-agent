...
namespace pinpoint {
    ...
    /**
     * @brief The inbound trace context, resolved into one decision.
     *
     * @see readInboundTrace
     */
    struct InboundTrace {
        /// The trace id parsed from the inbound header. Empty (and meaningless)
        /// unless `continued` is true — a new trace mints its own id.
        TraceId trace_id;
    ...
    /**
     * @brief Decides whether the inbound headers continue an existing trace,
     *        and parses the trace id when they do.
     *
     * The single place that decision is made: the sampler choice
     * (continue vs. new) and the context extraction below both run off this one
     * result, so they cannot disagree about what the request is. A request
     * continues an inbound trace only when the trace id parses **and** both
     * `Pinpoint-SpanID` and `Pinpoint-pSpanID` are present — only their
    ...
    public:
         ...
         *
         * @param trace_id The already-resolved trace id — readInboundTrace()'s
         *                 parsed id, or one generated by NewSpan. Must
         *                 be non-empty (NewSpan turns an empty/failed trace id
         *                 into a noop span). Moved into the span's SpanData.
         * @param continued `InboundTrace::continued` as returned by
         *                 readInboundTrace() — never a condition re-derived
         *                 here or at the call site. When false this is a brand
