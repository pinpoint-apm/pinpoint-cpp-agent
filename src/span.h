...
namespace pinpoint {
    ...
    public:
         ...
         * @param continued Whether `trace_id` came from the inbound
         *                 HEADER_TRACE_ID. When false this is a brand new root
         *                 trace, so no inbound header is read: adopting an
         *                 upstream span/parent id here would point the root
         *                 span at a parent that does not exist in this trace.
         *                 Mirrors Java's `if (!recorder.isRoot())` gate in
