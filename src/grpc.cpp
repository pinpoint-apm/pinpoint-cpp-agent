...
namespace pinpoint {
    ...
    bool GrpcAgent::registerAgentWithRetry() {
        ...
        ExponentialBackoff retry_backoff(retry_interval, 1.0,
                                         ...
                                         retry_interval * 2);
        // Spans and stats keep flowing during the wait, so the only visible
        // symptom is an agent missing from the server's agent list — easy to
        // read as a dead agent. The per-attempt failure lines say nothing
        // about it, so report the ongoing wait periodically.
        const auto started = std::chrono::steady_clock::now();
        ...
        while (!agent_->isExiting()) {
            ...
            const auto now = std::chrono::steady_clock::now();
            if (now >= next_log) {
                LOG_INFO("still waiting for agent registration after {}ms: spans and stats are being "
                         "sent, but this agent stays out of the server's agent list until the collector "
                         "accepts AgentInfo",
    ...
    bool GrpcAgent::run_ping_worker() {
        ...
        while (true) {
            ...
            if (write_and_await_ping_stream() == STREAM_DONE) {
                // The stream is gone (write failed, or it broke while idle).
                // Rebuild through the supervisor's restart delay instead of
                // reopening here: reopening falls into the ping_interval wait
                // below, so one failed ping would silence this agent for a
                // whole interval and the collector could mark it dead. Reachable
                // at start-up now that the workers no longer wait for a
                // successful registration to prove the collector is healthy.
                return false;
            }
