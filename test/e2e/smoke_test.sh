...
AGENT_TIMEOUT="${PINPOINT_IT_AGENT_TIMEOUT:-30}"
BASE_URL="http://${HOST}:${PORT}"
DOWNSTREAM_URL="http://${HOST}:${DOWNSTREAM_PORT}"
C_API_SCENARIO="${C_API_SCENARIO:-}"
FORK_SCENARIO="${FORK_SCENARIO:-}"
...
echo "============================================"
echo " Pinpoint C++ Agent - Correctness Smoke Test"
echo "============================================"
echo "Upstream:   $BASE_URL"
echo "Downstream: $DOWNSTREAM_URL"
echo "Collector:  ${PINPOINT_CPP_COLLECTOR_HOST:-unset}"
echo ""
...
echo ""
echo "C++ API and HTTP metadata"
http_request GET "$BASE_URL/features" \
    -H 'User-Agent: pinpoint-cpp-smoke' \
    -H 'Content-Type: application/json' \
    -H 'X-Request-ID: cpp-it-feature-001' \
...
# the above only covers a hop that receives s0, not one that forwards it.
...
# A filtered request gets a plain noop span (id 0); an unsampled one still
# carries a real id, so "traced" separates filtering from sampling.
...
if [[ -n "$C_API_SCENARIO" ]]; then
    ...
    if output=$("$C_API_SCENARIO" 2>&1); then
        if [[ "$output" == *'"status":"ok"'* && \
              "$output" == *'"context_injected":true'* && \
              "$output" == *'"async_complete":true'* ]]; then
            pass "C API lifecycle and feature scenario"
        else
            fail "C API scenario returned an unexpected payload" "$output"
        fi
    else
        fail "C API scenario failed" "$output"
    fi
fi

if [[ -n "$FORK_SCENARIO" ]]; then
    echo ""
    echo "Cold-create/fork lifecycle scenario"
    if output=$("$FORK_SCENARIO" 2>&1); then
        if [[ "$output" == *'"status":"ok"'* && \
              "$output" == *'"unique_agent_ids":true'* ]]; then
            pass "fork children received unique agent ids"
        else
            fail "fork scenario returned an unexpected payload" "$output"
        fi
    else
        fail "fork scenario failed" "$output"
    fi
fi
