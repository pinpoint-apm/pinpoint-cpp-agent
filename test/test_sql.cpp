...
TEST_F(SqlTest, NumericParameterReplacementTest) {
    ...
    EXPECT_EQ(result.normalized_sql, "SELECT * FROM users WHERE id = 1#");
    EXPECT_EQ(result.parameters, "123");
    
    result = normalizer_->normalize("SELECT * FROM users WHERE id = 123 AND age > 25");
    EXPECT_EQ(result.normalized_sql, "SELECT * FROM users WHERE id = 1# AND age > 2#");
...
TEST_F(SqlTest, NegativeNumberParameterTest) {
    ...
    EXPECT_EQ(result.normalized_sql, "SELECT * FROM users WHERE balance = 1#");
...
TEST_F(SqlTest, DecimalNumberParameterTest) {
    ...
    EXPECT_EQ(result.normalized_sql, "SELECT * FROM products WHERE price = 1#");
...
TEST_F(SqlTest, MultipleNumbersTest) {
    ...
    EXPECT_EQ(result.normalized_sql, "SELECT * FROM users WHERE age > 1# AND balance < 2# AND score = 3#");
...
TEST_F(SqlTest, StringLiteralReplacementTest) {
    ...
    EXPECT_EQ(result.normalized_sql, "SELECT * FROM users WHERE name = '1$'");
    ...
    EXPECT_EQ(result.normalized_sql, "INSERT INTO users (name, email) VALUES ('1$', '2$')");
...
TEST_F(SqlTest, MixedParametersTest) {
    ...
    EXPECT_EQ(result.normalized_sql, "SELECT * FROM users WHERE id = 1# AND name = '2$' AND age > 3# AND status = '4$'");
...
TEST_F(SqlTest, StringLiteralWithNumbersTest) {
    ...
    EXPECT_EQ(result.normalized_sql, "SELECT * FROM users WHERE phone = '1$' AND age > 2#");
...
TEST_F(SqlTest, ComplexSelectQueryTest) {
    ...
    EXPECT_TRUE(result.normalized_sql.find("u.created_at > '1$'") != std::string::npos);
    EXPECT_TRUE(result.normalized_sql.find("u.status = '2$'") != std::string::npos);
    EXPECT_TRUE(result.normalized_sql.find("u.age > 3#") != std::string::npos);
    EXPECT_TRUE(result.normalized_sql.find("COUNT(o.id) > 4#") != std::string::npos);
    EXPECT_TRUE(result.normalized_sql.find("LIMIT 5#") != std::string::npos);
...
TEST_F(SqlTest, ComplexInsertQueryTest) {
    ...
    auto result = normalizer_->normalize(complex_sql);
    // Should replace both string literals and numeric literals with proper indexing
    EXPECT_TRUE(result.normalized_sql.find("VALUES (1#, '2$', NOW(), 3#, '4$')") != std::string::npos);
...
TEST_F(SqlTest, OnePassNormalizationFlowTest) {
    ...
    EXPECT_TRUE(result.normalized_sql.find("u.age  >  1#") != std::string::npos);
    EXPECT_TRUE(result.normalized_sql.find("u.balance  =  2#") != std::string::npos);
    EXPECT_TRUE(result.normalized_sql.find("u.status  =  '3$'") != std::string::npos);
    EXPECT_TRUE(result.normalized_sql.find("u.created_at > '4$'") != std::string::npos);
    EXPECT_TRUE(result.normalized_sql.find("LIMIT    5#") != std::string::npos);
...
TEST_F(SqlTest, ParameterIndexingAccuracyTest) {
    ...
    EXPECT_EQ(result.normalized_sql, "SELECT 1#, '2$', 3#, '4$', 5#, 6#");
...
TEST_F(SqlTest, NestedQuotesAndCommentsTest) {
    ...
    EXPECT_EQ(result.normalized_sql, "SELECT '1$' FROM table WHERE id = 2# ");
