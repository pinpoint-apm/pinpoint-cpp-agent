...
| Request parameter recording | `HttpServletParameterExtractor`, `profiler.server.tracerequestparam`, `ClientRequestRecorder` / `InterceptorUtils.getHttpUrl`, `profiler.<plugin>.param` | **Same format as Java, opt-in instead of default on** — `Http.Server.RecordRequestParam` and `Http.Client.RecordUrlQuery` default to `false` because query strings carry tokens and ids; see [config.md](config.md#server-side-tracing) |
| Real-IP header resolution | `RealIpHeaderResolver`, `profiler.server.realipheader`, `profiler.server.realipemptyvalue` | **Same algorithm as Java, different default** — `Http.Server.RealIpHeader` defaults to `["X-Forwarded-For", "X-Real-Ip"]` for compatibility where Java trusts no header; see [config.md](config.md#server-side-tracing) |
