...
- At most **100 exceptions with call stacks are buffered per span**; further ones are dropped. The buffer does not shrink before `EndSpan()`, so once a link of the span's chain is dropped every later link is dropped with it, rather than being sent under an id whose earlier links were lost. Buffered exceptions are transmitted only at `EndSpan()` — a span kept open for a very long time delays them and grows memory. There is no per-chain depth cap (Java's `profiler.exceptiontrace.max.depth`, Go's `Error.MaxChainDepth`): the chain is flat, so the span cap is the only bound (see [Java parity](java_parity.md#per-span-exception-buffer-cap--declined)).
- **Exception chains are per span, not per span event, and flat.** Every call-stack `SetError()` on any span event of one span is a link of that span's one chain: the links share a single `exceptionId`, and each event that records a link carries the `ANNOTATION_EXCEPTION_ID` annotation once. Repeated `SetError()` calls — on one event, or on the events an exception passes through on its way up — are how a chain is built. This is what keeps an exception recorded on a nested event and again on the event that catches it one chain, charged to the `CallstackTraceNewThroughput` limiter once (Java's `ExceptionRecordingState` `CONTINUED` state; the Go agent's per-span `errorChains` lookup). Java and Go tell a cause from an unrelated exception by throwable identity and number the causes `PException.exceptionDepth` 0, 1, 2 …; this agent has no throwable identity, so it does neither: **every link is sent at depth 0**, and two unrelated exceptions recorded on one span share its id. A chain the limiter refused, or whose link no longer fits the buffer, stays refused for every later link on the span, whichever event records it. The refused links are still plain errors (`exceptionInfo`, failed transaction); only the call stacks are lost.
