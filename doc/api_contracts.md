...
- Every path into `SetError()` fails the transaction, including the ones that record nothing else: an **overflowed** span event (§5) marks the trace root without keeping the error, and an **unsampled** span (§11) marks its URL stat entry as failed. Recording a step error on an unsampled span **event** is deliberate: with sampling on, unsampled requests are the majority, and ignoring their step errors would bias the URL stat failure rate toward zero.
