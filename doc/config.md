...
| `CallstackTraceNewThroughput` | `PINPOINT_CPP_CALLSTACK_TRACE_NEW_THROUGHPUT` | int | `1000` | New exception chains admitted per second, agent-wide. `0` = unlimited; a negative value falls back to the default. Mirrors the Java agent's `profiler.exceptiontrace.new.throughput`. Only the **first** call stack of a chain is charged — a cause chain is never recorded half-way — and a refused chain is still reported as a plain error (`exceptionInfo` and the failed-transaction mark), it just carries no call stack. |
