...
The same `Grpc` channel options are applied to the agent, metadata, span, and stat gRPC channels. Both renewal options exist for collectors behind an L4 load balancer or scaled out in Kubernetes: without them an agent stays pinned to the backend it first connected to and new collector instances receive no traffic from already-running agents. Each of the agent's channels rotates independently, and the jitter keeps a fleet of agents from reconnecting in lockstep. Java-specific name resolver providers, custom interceptor injection, Netty channel type, channelz reporter wiring, retry/hedging service config, flow-control window, and write-buffer watermarks do not have a direct equivalent in the current C++ agent implementation.
