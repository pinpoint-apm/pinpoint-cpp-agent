...
ENTRYPOINT ["bash", "-ec", "preset=\"${1:-default}\"; case \"$preset\" in default|debug|debug-cached|vcpkg) cmake --preset \"$preset\"; cmake --build --preset \"$preset\" --parallel \"$(nproc)\"; ctest --preset \"$preset\" ;; asan|tsan|ubsan) cmake --preset \"$preset\"; cmake --build --preset \"$preset\"; ctest --preset \"$preset\" ;; coverage) cmake --preset coverage; cmake --build --preset coverage --parallel \"$(nproc)\" ;; profiling) cmake --preset profiling; cmake --build build/profiling --parallel \"$(nproc)\"; ctest --test-dir build/profiling --output-on-failure ;; bazel) bazel build //...; bazel test //test/... --test_output=errors ;; bazel-asan) bazel build --config=asan //...; bazel test --config=asan //test/... --test_output=errors ;; bazel-tsan) bazel build --config=tsan //...; bazel test --config=tsan //test/... --test_output=errors ;; bazel-ubsan) bazel build --config=ubsan //...; bazel test --config=ubsan //test/... --test_output=errors ;; bazel-profiling) bazel build --config=profiling //...; bazel test --config=profiling //test/... --test_output=errors ;; help|--help|-h) echo 'Presets: default, debug, debug-cached, vcpkg, asan, tsan, ubsan, coverage, profiling, bazel, bazel-asan, bazel-tsan, bazel-ubsan, bazel-profiling' ;; *) echo \"Unknown preset: $preset\" >&2; exit 2 ;; esac", "pinpoint-cpp-agent-test"]
CMD ["default"]
